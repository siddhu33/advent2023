...
function part1() {
    ...
    lines.forEach(line => {
        ...
        const winningNumbers = winners.split(" ").map(n => parseInt(n, 10)).filter(n => !isNaN(n))
        const winningSet = new Set<number>(winningNumbers)
        const cardNumbers = cards.split(" ").map(n => parseInt(n, 10)).filter(n => !isNaN(n))
...
function part2() {
    const numCopies: copies = Object.fromEntries([...Array(lines.length).keys()].map(x => [x, 1]))
