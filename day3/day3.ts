...
function part1() {
  ...
  for (const symbolLocation of symbolLocations) {
    for (const numberLocation of numberLocations) {
      if (symbolLocation.line === numberLocation.line) {
        if (
          symbolLocation.index >= numberLocation.start &&
          symbolLocation.index <= numberLocation.end
        ) {
  ...
  const allParts = Array.from(numbersSelected.values())
    .map((v) => JSON.parse(v))
    .map((x) => x.value)
    .reduce((a, b) => a + b);
